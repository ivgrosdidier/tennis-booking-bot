...
# Cloud Run automatically sets K_SERVICE. If it's missing, we are on Local.
IS_CLOUD = os.getenv('K_SERVICE') is not None

# firebase initialization 
if not firebase_admin._apps:
    if IS_CLOUD:
        # In Cloud Run, use the "Default" identity of the server
        cred = credentials.ApplicationDefault()
    else:
        # On Local, use your downloaded JSON key
...
# fix insecure transport error for local testing (removed in production)
if not IS_CLOUD:
    # ONLY allow insecure (HTTP) transport when testing on your laptop
    os.environ["OAUTHLIB_INSECURE_TRANSPORT"] = "1"
    print("[Auth] Local mode: Insecure Transport Enabled")
